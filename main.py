...
class CoinsApp(App):
    def build(self):
        ...
        self.player_id = TextInput(
            ...
            size_hint_y=None, height=120, input_filter='int'
        )

        self.btn_3 = Button(text="START INJECTION", background_color=(0.1, 0.7, 0.2, 1), bold=True)
        self.btn_3.bind(on_press=self.start_process)
    ...
    def update_timer(self, dt):
        if self.seconds > 0:
            ...
            self.status.text = f"PROCESSING... {mins:02d}:{secs:02d}"
            self.seconds -= 1
        else:
