...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS tasks (
        ...
        date TEXT, xp_reward INTEGER DEFAULT 10,
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS achievements (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT, desc TEXT,
        earned INTEGER DEFAULT 0, date TEXT
    )''')
    c.execute('''CREATE TABLE IF NOT EXISTS boss (
        id INTEGER PRIMARY KEY,
        name TEXT, hp INTEGER DEFAULT 100,
        max_hp INTEGER DEFAULT 100, date TEXT
    )''')
    c.execute('SELECT COUNT(*) FROM profile')
    if c.fetchone()[0] == 0:
        sid = 'SM-' + hashlib.md5(str(datetime.datetime.now()).encode()).hexdigest()[:8].upper()
        c.execute('INSERT INTO profile (shadow_id,level,xp,rank,streak,last_login,title,army,power) VALUES (?,1,0,"E",0,?,"مبتدئ الظل",0,100)',
                  (sid, str(datetime.date.today())))
    c.execute('SELECT COUNT(*) FROM achievements')
    if c.fetchone()[0] == 0:
        achs = [
            ('اول_ظل',       'اكمل مهمتك الاولى',          0, ''),
            ('جيش_x10',      'اكمل 10 مهام',                0, ''),
            ('جيش_x50',      'اكمل 50 مهمة',                0, ''),
            ('محارب_اسبوع',  'حافظ على سلسلة 7 ايام',       0, ''),
            ('سيد_الظل',     'ابلغ رتبة S',                  0, ''),
            ('حارس_الكود',   'احفظ 10 مقتطفات كود',          0, ''),
            ('بومة_الليل',   'سجل الدخول بعد منتصف الليل',   0, ''),
    ...
    if c.fetchone()[0] == 0:
        bosses = ['وحش الظل', 'الشبح الداكن', 'سالك الفراغ', 'رعب الليل', 'رب الهاوية']
...
def add_xp(amount):
    ...
    titles = {'E': 'مبتدئ الظل', 'D': 'سالك الظل', 'C': 'فارس الظل',
              'B': 'صائد الظل', 'A': 'سيد الظل', 'S': 'ملك الظل'}
    rank = 'E'
...
def make_header(title, back_cb=None):
    ...
    bg(header, C['header'])
    if back_cb:
        b = styled_btn('رجوع', color=(0,0,0,0), fg=C['purple'],
...
class SplashScreen(Screen):
    def __init__(self, **kwargs):
        ...
        overlay.add_widget(lbl('ملك ضلال', size=44, color=C['purple'], bold=True,
                               pos_hint={'center_x':0.5,'center_y':0.62},
                               size_hint=(1,None), height=dp(65)))
        overlay.add_widget(lbl('Shadow Monarch v3.0', size=14, color=C['sub'],
                               pos_hint={'center_x':0.5,'center_y':0.51},
                               size_hint=(1,None), height=dp(28)))
        overlay.add_widget(lbl('... استيقظ', size=13, color=(0.4,0.2,0.7,1),
...
class LockScreen(Screen):
    def __init__(self, **kwargs):
        ...
        overlay.bind(pos=lambda i,v: setattr(self._ov,'pos',v),
                     size=lambda i,v: setattr(self._ov,'size',v))
        overlay.add_widget(lbl('ملك ضلال', size=36, color=C['purple'], bold=True,
                               pos_hint={'center_x':0.5,'center_y':0.86},
                               size_hint=(1,None), height=dp(52)))
        overlay.add_widget(lbl('ادخل الرمز السري', size=13, color=C['sub'],
    ...
    def on_btn(self, btn):
        ...
        if t == '<':
            ...
            if self.pin_entered == '20057':
                ...
                self.err_lbl.text = 'رمز خاطئ! حاول مجددا'
                self.pin_entered = ''
    ...
    def _update_login(self):
        try:
            ...
            if 0 <= datetime.datetime.now().hour < 4:
                con = sqlite3.connect(DB)
                c = con.cursor()
                c.execute("UPDATE achievements SET earned=1,date=? WHERE name='بومة_الليل' AND earned=0",
                          (today,))
                con.commit()
                con.close()
        except Exception:
...
class MainScreen(Screen):
    def __init__(self, **kwargs):
        ...
        overlay.bind(pos=lambda i,v: setattr(self._ov,'pos',v),
                     size=lambda i,v: setattr(self._ov,'size',v))
        header = BoxLayout(pos_hint={'x':0,'top':1}, size_hint=(1,0.09), padding=dp(12))
        bg(header, C['header'])
        header.add_widget(lbl('ملك ضلال', size=17, color=C['purple'], bold=True))
        self.clock_lbl = lbl('', size=13, color=C['sub'], size_hint=(None,1), width=dp(80))
        header.add_widget(self.clock_lbl)
        overlay.add_widget(header)
        pc = BoxLayout(orientation='vertical',
                       pos_hint={'center_x':0.5,'center_y':0.76},
                       size_hint=(0.93,None), height=dp(100),
                       padding=dp(10), spacing=dp(3))
        bg(pc, C['card'], radius=14)
        self.rank_lbl   = lbl('الرتبة: E',          size=13, color=C['gold'], bold=True)
        self.title_lbl  = lbl('مبتدئ الظل',          size=12, color=C['sub'])
        self.level_lbl  = lbl('المستوى 1  |  XP: 0', size=12, color=C['text'])
        self.streak_lbl = lbl('السلسلة: 0 يوم',      size=11, color=C['green'])
        self.army_lbl   = lbl('جيش الظل: 0',         size=11, color=C['purple'])
        ...
        for w in [self.rank_lbl, self.title_lbl, self.level_lbl,
            ...
            pc.add_widget(w)
        overlay.add_widget(pc)
        bc = BoxLayout(orientation='vertical',
                       pos_hint={'center_x':0.5,'center_y':0.59},
                       size_hint=(0.93,None), height=dp(55),
                       padding=dp(8), spacing=dp(4))
        bg(bc, (0.15,0.0,0.05,0.9), radius=12)
        self.boss_lbl = lbl('الرئيس اليومي: جاري التحميل...', size=13, color=C['red'], bold=True)
        ...
        overlay.add_widget(bc)
        scroll = ScrollView(pos_hint={'x':0,'y':0.0}, size_hint=(1,0.52))
        ...
        tools = [
            ...
            ('مؤقت بومودورو',   'pomodoro', (0.2,0.03,0.05,0.9),    '⏱'),
    ...
    def refresh_profile(self, dt=None):
        try:
            p = get_profile()
            self.rank_lbl.text   = f'الرتبة: {p["rank"]}'
            self.rank_lbl.color  = RANK_COLORS.get(p['rank'], C['gold'])
            self.title_lbl.text  = p['title']
            xp_need = p['level'] * 100
            self.level_lbl.text  = f'المستوى {p["level"]}  |  XP: {p["xp"]}/{xp_need}'
            self.xp_bar.max      = xp_need
            self.xp_bar.value    = p['xp']
            self.streak_lbl.text = f'السلسلة: {p["streak"]} يوم'
            self.army_lbl.text   = f'جيش الظل: {p["army"]}'
            con = sqlite3.connect(DB)
            c = con.cursor()
            today = str(datetime.date.today())
            ...
            if boss:
                if boss[4] != today:
                    bosses = ['وحش الظل','الشبح الداكن','سالك الفراغ','رعب الليل','رب الهاوية']
                    c.execute('UPDATE boss SET name=?,hp=100,max_hp=100,date=? WHERE id=1',
                              (random.choice(bosses), today))
                    con.commit()
                    c.execute('SELECT * FROM boss WHERE id=1')
                    boss = c.fetchone()
                self.boss_lbl.text  = f'الرئيس: {boss[1]}  ❤ {boss[2]}/{boss[3]}'
...
class TasksScreen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('المهام اليومية ⚔', back_cb=lambda x: go_back(self.manager)))
        ...
        self.task_inp = TextInput(hint_text='مهمة جديدة...',
        ...
        try:
            ...
            for t in tasks:
                ...
                l = lbl(t[1], size=13, color=C['green'] if done else C['text'],
                        halign='right', text_size=(Window.width*0.52, None))
            ...
            if not done:
                ...
                if row:
                    ...
                    if hp_row and hp_row[0] == 0:
                        today = str(datetime.date.today())
                        c.execute("UPDATE achievements SET earned=1,date=? WHERE name='قاتل_الرئيس' AND earned=0", (today,))
            con.commit()
            con.close()
            self.refresh()
            try:
...
class NotesScreen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('الملاحظات 📝', back_cb=lambda x: go_back(self.manager)))
        ...
        self.note_inp = TextInput(hint_text='اكتب ملاحظة...',
                                  background_color=C['card2'][:3]+(1,),
    ...
    def refresh(self):
        ...
        try:
            ...
            for n in notes:
                ...
                l = lbl(preview, size=13, halign='right', text_size=(Window.width*0.65, None))
...
class SnippetsScreen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('مقتطفات الكود 💾', back_cb=lambda x: go_back(self.manager)))
        ...
        root.add_widget(self.scroll)
        inp = BoxLayout(orientation='vertical', pos_hint={'x':0,'y':0}, size_hint=(1,0.24),
                        padding=dp(8), spacing=dp(6))
        self.t_inp = TextInput(hint_text='العنوان...', multiline=False,
                               background_color=C['card2'][:3]+(1,), foreground_color=C['text'],
                               font_size=dp(13), size_hint_y=None, height=dp(38))
        self.c_inp = TextInput(hint_text='الكود هنا...',
                               background_color=(0.03,0.0,0.1,1), foreground_color=(0.4,1,0.4,1),
                               font_size=dp(12))
        sb = styled_btn('حفظ المقتطف', color=C['purple2'], size_hint_y=None, height=dp(35))
    ...
    def refresh(self):
        ...
        try:
            ...
            for s in snips:
                ...
                info.add_widget(lbl(s[1], size=13, bold=True, color=C['purple'], halign='right'))
                preview = s[2][:40] + '...' if len(s[2]) > 40 else s[2]
                info.add_widget(lbl(preview, size=11, color=(0.4,1,0.4,1), halign='left'))
                cb = styled_btn('نسخ', color=(0,0.35,0.15,1), size_hint=(None,1), width=dp(55))
    ...
    def add_snip(self, *a):
        ...
        if t and c_text:
            try:
                ...
                if cnt >= 10:
                    today = str(datetime.date.today())
                    cur.execute("UPDATE achievements SET earned=1,date=? WHERE name='حارس_الكود' AND earned=0", (today,))
                    con.commit()
                con.close()
                self.t_inp.text = ''
                self.c_inp.text = ''
...
class CalcScreen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('الآلة الحاسبة 🔢', back_cb=lambda x: go_back(self.manager)))
    ...
    def on_calc(self, btn):
        ...
        if t == 'C':
            ...
            try:
                ...
                self.sub_disp.text = 'ليس عددا صحيحا'
        elif t == '()':
            self.expr += '('
...
class PomodoroScreen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('مؤقت بومودورو ⏱', back_cb=lambda x: go_back(self.manager)))
        self.mode_lbl = lbl('تركيز', size=20, color=C['red'], bold=True,
                            pos_hint={'center_x':0.5,'center_y':0.80},
                            size_hint=(1,None), height=dp(32))
        self.timer_lbl = lbl('25:00', size=66, color=C['text'], bold=True,
                             pos_hint={'center_x':0.5,'center_y':0.65},
                             size_hint=(1,None), height=dp(85))
        self.sess_lbl = lbl('الجلسات: 0', size=14, color=C['sub'],
                            pos_hint={'center_x':0.5,'center_y':0.54},
                            size_hint=(1,None), height=dp(28))
        btns = BoxLayout(pos_hint={'center_x':0.5,'center_y':0.43},
                         size_hint=(0.8,None), height=dp(55), spacing=dp(12))
        self.start_btn = styled_btn('ابدأ', color=C['green'][:3]+(1,), font_size=19)
        self.start_btn.bind(on_press=self.toggle)
        reset_btn = styled_btn('اعادة', color=C['red'][:3]+(0.8,), font_size=19)
    ...
    def tick(self, dt):
        if self.seconds_left > 0:
            ...
            if not self.is_break:
                self.sessions += 1
                self.sess_lbl.text = f'الجلسات: {self.sessions}'
                add_xp(25)
                self.is_break = True
                self.seconds_left = 5*60
                self.mode_lbl.text = 'استراحة'
                self.mode_lbl.color = C['green']
            else:
                self.is_break = False
                self.seconds_left = 25*60
                self.mode_lbl.text = 'تركيز'
...
class PassGenScreen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('مولد كلمات السر 🔑', back_cb=lambda x: go_back(self.manager)))
        self.result_lbl = lbl('اضغط توليد', size=15, color=(0.5,1,0.5,1),
                              pos_hint={'center_x':0.5,'center_y':0.73},
                              size_hint=(0.9,None), height=dp(48))
        self.len_lbl = lbl('الطول: 16', size=14, color=C['text'],
                           pos_hint={'center_x':0.5,'center_y':0.62},
                           size_hint=(1,None), height=dp(25))
        self.slider = Slider(min=8, max=32, value=16, step=1,
                             pos_hint={'center_x':0.5,'center_y':0.54},
                             size_hint=(0.85,None), height=dp(40))
        self.slider.bind(value=lambda s,v: setattr(self.len_lbl,'text',f'الطول: {int(v)}'))
        ...
        for b in [self.bu, self.bd, self.bs]:
            opts.add_widget(b)
        gen_btn = styled_btn('توليد', color=C['purple2'], font_size=19,
                             size_hint=(0.6,None), height=dp(54),
                             pos_hint={'center_x':0.5,'center_y':0.29})
        gen_btn.bind(on_press=self.generate)
        copy_btn = styled_btn('نسخ', color=(0,0.35,0.15,1), font_size=14,
...
class RegexScreen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('اختبار Regex 🔍', back_cb=lambda x: go_back(self.manager)))
        content = BoxLayout(orientation='vertical', pos_hint={'x':0,'y':0},
                            size_hint=(1,0.93), padding=dp(10), spacing=dp(8))
        content.add_widget(lbl('النمط (Pattern):', size=13, color=C['purple'],
                               size_hint_y=None, height=dp(22), halign='right'))
        self.pat = TextInput(hint_text=r'\d+', multiline=False,
                             background_color=C['card2'][:3]+(1,),
                             foreground_color=(1,1,0,1), font_size=dp(14),
                             size_hint_y=None, height=dp(42))
        content.add_widget(self.pat)
        content.add_widget(lbl('النص للاختبار:', size=13, color=C['purple'],
                               size_hint_y=None, height=dp(22), halign='right'))
        self.test_inp = TextInput(hint_text='ادخل النص هنا...',
                                  background_color=C['card2'][:3]+(1,),
                                  foreground_color=C['text'], font_size=dp(13),
                                  size_hint_y=None, height=dp(80))
        content.add_widget(self.test_inp)
        tb = styled_btn('اختبار', color=C['purple2'], font_size=17,
    ...
    def test_regex(self, *a):
        try:
            ...
            if m:
                ...
                self.res.text = f'وجدت {len(m)} تطابق:\n' + '\n'.join(str(x) for x in m[:12])
            else:
                self.res.color = C['red']
                self.res.text = 'لا يوجد تطابق'
...
class JsonScreen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('منسق JSON 📋', back_cb=lambda x: go_back(self.manager)))
        content = BoxLayout(orientation='vertical', pos_hint={'x':0,'y':0},
                            size_hint=(1,0.93), padding=dp(10), spacing=dp(8))
        self.inp = TextInput(hint_text='الصق JSON هنا...',
                             background_color=(0.03,0.0,0.1,1),
                             foreground_color=C['text'], font_size=dp(12))
        btns = BoxLayout(size_hint_y=None, height=dp(46), spacing=dp(8))
        for btn_txt, cb, col in [
            ('تنسيق', self.fmt, C['purple2']),
            ('تحقق',  self.val, (0,0.3,0.15,1)),
            ('نسخ',   lambda x: Clipboard.copy(self.inp.text), (0.2,0.1,0,1)),
        ]:
    ...
    def fmt(self, *a):
        try:
            ...
            self.status.text = 'تم التنسيق بنجاح'
        except Exception as e:
...
class BaseConvScreen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('محول القواعد 🔄', back_cb=lambda x: go_back(self.manager)))
        content = BoxLayout(orientation='vertical', pos_hint={'x':0,'y':0},
                            size_hint=(1,0.93), padding=dp(15), spacing=dp(10))
        self.fields = {}
        for label_txt, key, color in [
            ('عشري (Decimal)',  'dec', C['text']),
            ('ست عشري (Hex)',  'hex', (1,0.8,0,1)),
            ('ثنائي (Binary)', 'bin', (0.4,1,0.4,1)),
            ('ثماني (Octal)',  'oct', (0.6,0.8,1,1)),
        ]:
        ...
        cb = styled_btn('تحويل', color=C['purple2'], font_size=17,
...
class HashGenScreen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('مولد الهاش 🔐', back_cb=lambda x: go_back(self.manager)))
        content = BoxLayout(orientation='vertical', pos_hint={'x':0,'y':0},
                            size_hint=(1,0.93), padding=dp(12), spacing=dp(8))
        content.add_widget(lbl('ادخل النص:', size=13, color=C['purple'],
                               size_hint_y=None, height=dp(22), halign='right'))
        self.inp = TextInput(hint_text='النص هنا...',
                             background_color=C['card2'][:3]+(1,), foreground_color=C['text'],
                             font_size=dp(13), size_hint_y=None, height=dp(70))
        content.add_widget(self.inp)
        gen_btn = styled_btn('توليد الهاش', color=C['purple2'], font_size=16,
...
class UnitConvScreen(Screen):
    ...
    UNITS = {
        'الطول':          {'متر':1.0,'كيلومتر':1000.0,'سنتيمتر':0.01,'مليمتر':0.001,'ميل':1609.34,'قدم':0.3048,'بوصة':0.0254},
        'الوزن':          {'كيلوغرام':1.0,'غرام':0.001,'مليغرام':0.000001,'طن':1000.0,'باوند':0.453592,'اونصة':0.0283495},
        'درجة الحرارة':   {'مئوي C':'C','فهرنهايت F':'F','كلفن K':'K'},
        'المساحة':        {'متر مربع':1.0,'كم مربع':1000000.0,'هكتار':10000.0,'قدم مربع':0.092903,'ميل مربع':2589988.0},
        'السرعة':         {'م/ث':1.0,'كم/س':0.277778,'ميل/س':0.44704,'عقدة':0.514444},
    }
    ...
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('محول الوحدات 📐', back_cb=lambda x: go_back(self.manager)))
        content = BoxLayout(orientation='vertical', pos_hint={'x':0,'y':0},
        ...
        content.add_widget(lbl('القيمة:', size=13, color=C['purple'], size_hint_y=None, height=dp(20), halign='right'))
        ...
        content.add_widget(lbl('من:', size=13, color=C['purple'], size_hint_y=None, height=dp(20), halign='right'))
        ...
        content.add_widget(lbl('الى:', size=13, color=C['purple'], size_hint_y=None, height=dp(20), halign='right'))
        ...
        conv_btn = styled_btn('تحويل', color=C['purple2'], font_size=17, size_hint_y=None, height=dp(50))
    ...
    def _build_btns(self, box, which):
        ...
        for u in self.UNITS.get(self.cur_cat, {}).keys():
            b = styled_btn(u, color=C['card2'], font_size=11, size_hint=(None,1), width=dp(100))
    ...
    def convert(self, *a):
        try:
            ...
            if not self.from_unit or not self.to_unit:
                self.result_lbl.text = 'اختر وحدتي التحويل'
                return
            ...
            self.result_lbl.text = f'النتيجة: {r:.6g}'
        except Exception:
            self.result_lbl.text = 'خطأ في الادخال'
...
class Base64Screen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('مشفر النصوص 🔒', back_cb=lambda x: go_back(self.manager)))
        content = BoxLayout(orientation='vertical', pos_hint={'x':0,'y':0},
                            size_hint=(1,0.93), padding=dp(12), spacing=dp(8))
        content.add_widget(lbl('النص المدخل:', size=13, color=C['purple'],
                               size_hint_y=None, height=dp(22), halign='right'))
        self.inp = TextInput(hint_text='ادخل النص هنا...',
                             background_color=C['card2'][:3]+(1,), foreground_color=C['text'],
                             font_size=dp(13), size_hint_y=None, height=dp(90))
        content.add_widget(self.inp)
        btns = BoxLayout(size_hint_y=None, height=dp(46), spacing=dp(6))
        for btn_txt, cb, col in [
            ('تشفير B64',  self.encode_b64,  C['purple2']),
            ('فك B64',     self.decode_b64,  (0,0.3,0.15,1)),
            ('URL تشفير',  self.url_encode,  (0.2,0.1,0,1)),
            ('URL فك',     self.url_decode,  (0.1,0.15,0.05,1)),
        ]:
            b = styled_btn(btn_txt, color=col, font_size=12)
            b.bind(on_press=cb)
            btns.add_widget(b)
        content.add_widget(btns)
        self.out = TextInput(hint_text='النتيجة ستظهر هنا...',
                             background_color=(0.03,0.0,0.1,1), foreground_color=(0.4,1,0.4,1),
                             font_size=dp(13), size_hint_y=None, height=dp(120))
        content.add_widget(self.out)
        copy_btn = styled_btn('نسخ النتيجة', color=(0,0.35,0.15,1), font_size=14,
    ...
    def encode_b64(self, *a):
        try:
            self.out.text = base64.b64encode(self.inp.text.encode('utf-8')).decode('utf-8')
        except Exception as e:
            self.out.text = f'Error: {e}'

    def decode_b64(self, *a):
        try:
            self.out.text = base64.b64decode(self.inp.text.encode('utf-8')).decode('utf-8')
        except Exception as e:
            self.out.text = f'Error: {e}'
...
class WorldClockScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('الساعة العالمية 🌍', back_cb=lambda x: self._go_back()))
        ...
        for name, _ in self.ZONES:
            ...
            bg(row, C['card'], radius=8)
            name_lbl = lbl(name, size=13, color=C['purple'], halign='right', size_hint=(0.55,1))
            time_lbl = lbl('--:--:--', size=20, color=C['gold'], bold=True, halign='left', size_hint=(0.45,1))
...
class AchieveScreen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('الانجازات 🏆', back_cb=lambda x: go_back(self.manager)))
    ...
    def on_enter(self):
        ...
        try:
            ...
            con.close()
            names = {
                'اول_ظل':'اول ظل','جيش_x10':'جيش x10','جيش_x50':'جيش x50',
                'محارب_اسبوع':'محارب الاسبوع','سيد_الظل':'سيد الظل',
                'حارس_الكود':'حارس الكود','بومة_الليل':'بومة الليل','قاتل_الرئيس':'قاتل الرئيس',
            }
            for a in achs:
                earned = a[3]
                ...
                info = BoxLayout(orientation='vertical')
                disp = names.get(a[1], a[1])
                pfx = '✅ ' if earned else '🔒 '
                info.add_widget(lbl(pfx+disp, size=14, bold=True,
                                    color=C['gold'] if earned else C['sub'], halign='right'))
                info.add_widget(lbl(a[2], size=11, color=C['text'], halign='right'))
                if earned and a[4]:
                    info.add_widget(lbl(f'بتاريخ: {a[4]}', size=10, color=C['green'], halign='right'))
                card.add_widget(info)
...
class ArmyScreen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('جيش الظل 👥', back_cb=lambda x: go_back(self.manager)))
        ...
        self.army_lbl   = lbl('الجيش: 0',       size=30, color=C['purple'], bold=True)
        self.rank_lbl   = lbl('الرتبة: E',       size=20, color=C['gold'])
        self.level_lbl  = lbl('المستوى: 1',      size=16, color=C['text'])
        self.streak_lbl = lbl('السلسلة: 0 يوم',  size=14, color=C['green'])
        ...
        nexts = {'E':'ابلغ المستوى 5 - رتبة D','D':'ابلغ المستوى 10 - رتبة C',
                 'C':'ابلغ المستوى 20 - رتبة B','B':'ابلغ المستوى 35 - رتبة A',
                 'A':'ابلغ المستوى 50 - رتبة S','S':'اعلى رتبة تم بلوغها!'}
    ...
    def on_enter(self):
        try:
            p = get_profile()
            self.army_lbl.text   = f'الجيش: {p["army"]}'
            self.rank_lbl.text   = f'الرتبة: {p["rank"]}'
            self.rank_lbl.color  = RANK_COLORS.get(p['rank'], C['gold'])
            self.level_lbl.text  = f'المستوى: {p["level"]}'
            self.streak_lbl.text = f'السلسلة: {p["streak"]} يوم'
            self.id_lbl.text     = f'Shadow ID: {p["shadow_id"]}'
            xp_need = p['level'] * 100
            self.xp_bar.max   = xp_need
            self.xp_bar.value = p['xp']
            self.next_info.text = self._nexts.get(p['rank'], '')
        except Exception:
            pass
...
class SkillsScreen(Screen):
    def __init__(self, **kwargs):
        ...
        root = FloatLayout()
        make_screen_bg(self)
        root.add_widget(make_header('المهارات ⚡', back_cb=lambda x: go_back(self.manager)))
        self.scroll = ScrollView(pos_hint={'x':0,'y':0}, size_hint=(1,0.93))
        self.sl = BoxLayout(orientation='vertical', size_hint_y=None, spacing=dp(8), padding=dp(10))
        self.sl.bind(minimum_height=self.sl.setter('height'))
        self.scroll.add_widget(self.sl)
    ...
    def on_enter(self):
        ...
        try:
            ...
            level = p.get('level', 1)
            skills = [
                ('خطوة الظل',   'E',  1,  'حركة اساسية في الظلام'),
                ('رداء الظل',   'E',  3,  'احجب كل مشتتات الانتباه'),
                ('رؤية الظلام', 'D',  5,  'شاهد الفرص التي يفوتها الاخرون'),
                ('جيش الظل',    'D',  8,  'قد جيش مهامك'),
                ('ضربة الفراغ', 'C',  12, 'انجز المهام في لحظة'),
                ('مجال الظل',   'C',  18, 'سيطر على بيئتك الكاملة'),
                ('ارادة الملك', 'B',  25, 'اثن الواقع نحو اهدافك'),
                ('انهض',        'A',  40, 'استدع اقصى انتاجيتك'),
                ('ملك الظل',    'S',  50, 'القوة المطلقة مفعلة'),
            ]
            ...
            for name, req_rank, req_lvl, desc in skills:
                ...
                info.add_widget(lbl(pfx+name, size=14, bold=True,
                                    color=C['gold'] if unlocked else C['sub'], halign='right'))
                info.add_widget(lbl(desc, size=11, color=C['text'], halign='right'))
                req = lbl(f'رتبة {req_rank} | مستوى {req_lvl}', size=10,
